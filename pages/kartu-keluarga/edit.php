...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE kartu_keluarga SET nomor_kk = ?, alamat = ?, rt_rw = ?, id_wilayah = ? WHERE id = ?";
    $stmt = mysqli_prepare($koneksi, $query);
    mysqli_stmt_bind_param($stmt, "sssii", $nomor_kk, $alamat, $rt_rw, $id_wilayah, $id);
...
<head>
    ...
    <title>Edit Kartu Keluarga - SINDAGA</title>
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <div class="mb-8">
                    <div class="flex justify-between items-center">
                        <div>
                            <h1 class="text-2xl font-bold text-white">Edit Kartu Keluarga</h1>
                            <p class="text-base text-gray-400 mt-1">Edit data kartu keluarga yang sudah ada</p>
                        </div>
                        <a href="index.php"
                ...
                <div class="bg-dark-200 rounded-xl border border-dark-300 shadow-lg">
                    <div class="border-b border-dark-300 p-8">
                        <h2 class="text-2xl font-semibold text-white">Formulir Edit KK</h2>
                        <p class="mt-2 text-base text-gray-400">Perbarui informasi kartu keluarga</p>
                    </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <textarea name="alamat" required rows="3"
                                        class="w-full rounded-lg bg-dark-100 border-dark-300 text-gray-300 
                                               text-base py-3.5 px-5 placeholder-gray-500
                                               focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"><?= $data['alamat'] ?></textarea>
                                </div>
                            </div>
                        </div>
                                ...
                                <div>
                                    ...
                                    <select name="id_wilayah" required
                                        ...
                                        <?php while ($row = mysqli_fetch_assoc($result_wilayah)): ?>
                                            <option value="<?= $row['id'] ?>"
                                                <?= $data['id_wilayah'] == $row['id'] ? 'selected' : '' ?>>
                                                <?= $row['nama_wilayah'] ?> (<?= $row['level_wilayah'] ?>)
                                            </option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
