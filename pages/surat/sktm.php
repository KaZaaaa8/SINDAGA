...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $data_penduduk = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));

    // Get absolute paths
    $template_path = realpath("../../Surat/SK_SKTM.docx");
    $output_dir = realpath("../../Surat/output");
    $output_filename = "surat_sktm_" . time() . ".docx";
    ...
    $template->setValue('tanggal_lahir', date('d-m-Y', strtotime($data_penduduk['tanggal_lahir'])));
    $template->setValue('jenis_kelamin', $data_penduduk['jenis_kelamin'] == 'L' ? 'Laki-laki' : 'Perempuan');
    $template->setValue('suku', $suku);
    ...
    if (file_exists($output_path)) {
        header("Location: index.php?success=1&type=sktm&file=" . $output_filename);
        exit();
    }
}
?>
...
<head>
    ...
    <title>Surat Keterangan Tidak Mampu - SINDAGA</title>
            ...
            <div class="max-w-5xl mx-auto">
                ...
                <div class="mb-8">
                    <div class="flex justify-between items-center">
                        <div>
                            <h1 class="text-2xl font-bold text-white">Surat Keterangan Tidak Mampu</h1>
                            <p class="text-base text-gray-400 mt-1">Buat surat keterangan tidak mampu baru</p>
                        </div>
                        <a href="index.php"
                ...
                <div class="bg-dark-200 rounded-xl border border-dark-300 shadow-lg">
                    <div class="border-b border-dark-300 p-8">
                        <h2 class="text-2xl font-semibold text-white">Formulir SKTM</h2>
                    ...
                    <form method="POST" class="p-8 space-y-12">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-8">
                                ...
                                <div>
                                    <label class="block text-base font-medium text-gray-300 mb-3">
                                        Suku <span class="text-rose-500">*</span>
                                    </label>
                                    <input type="text" name="suku" required value="Indonesia"
                                        class="w-full rounded-lg bg-dark-100 border-dark-300 text-gray-300 
                                               text-base py-3.5 px-5 placeholder-gray-500
                                               focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        <!-- Keperluan -->
                        <div>
                            <h3 class="text-lg font-semibold text-indigo-400 mb-8 pb-3 border-b border-dark-300">Keperluan</h3>
                            <div>
                                <label class="block text-base font-medium text-gray-300 mb-3">
                                ...
                                <textarea name="keperluan" required rows="3"
                                    class="w-full rounded-lg bg-dark-100 border-dark-300 text-gray-300 
                                           text-base py-3.5 px-5 placeholder-gray-500
                                           focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                    placeholder="Masukkan keperluan pembuatan SKTM"></textarea>
                            </div>
                        </div>
